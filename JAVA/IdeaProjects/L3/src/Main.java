...
public class Main {
    ...
    static File cName=new File("Customer Names.txt");          //creating files for each arrays
    static File cPassword=new File("Customer Passwords.txt");
    static File skName=new File("ShopKeeper Names.txt");
    static File skPassword =new File("Shopkeeper Passwords.txt");
    static File skIDs =new File("Shopkeeper ID numbers.txt");
    static File productName =new File("Products.txt");
    static File productCounts =new File("Quantity of Products.txt");
    static File productRates =new File("Rate of Products.txt");
    static File Wallets =new File("Wallet.txt");
    static File shopKeeperCount=new File("Count of the ShopKeeper created" );
    static File customerCount=new File("Count of the Customer created");
    ...
    public static void signUp(boolean isShopkeeper)throws Exception{
        if(isShopkeeper){                   //is a shop keeper
            sk++;
            oos=new ObjectOutputStream(new FileOutputStream(shopKeeperCount));//uploading count of shopkeeper present as a file as well
            oos.writeObject(sk);
            oos.close();
            ID++;
            ...
            //for(i=0;i<sk;i++){
                ...
                IDs[sk-1].setIDs(ID);
                oos=new ObjectOutputStream(new FileOutputStream(skIDs));
                oos.writeObject(ID);
                oos.close();
            //}
            ...
            //for(i=0;i<sk;i++){
                ...
                oos=new ObjectOutputStream(new FileOutputStream(skName));
            ...
            //for(i=0;i<sk;i++){
                ...
                oos=new ObjectOutputStream(new FileOutputStream(skPassword));
            ...
            c++;
            oos=new ObjectOutputStream(new FileOutputStream(customerCount));//uploading count of customer to a file
            oos.writeObject(c);
            oos.close();
            ...
            //for(i=0;i<c;i++){
                ...
                oos=new ObjectOutputStream(new FileOutputStream(cName));
                oos.writeObject(name);
                oos.close();
            //}
            ...
            //for(i=0;i<c;i++){
                ...
                oos=new ObjectOutputStream(new FileOutputStream(cPassword));
                oos.writeObject(password);
                oos.close();
            //}
        }
...
public static void shopKeeperLogIn (int tId,String tname,String tpassword)throws Exception{
        int i;
        ois=new ObjectInputStream(new FileInputStream(shopKeeperCount));//copying val back from file to sk
        ...
        ois=new ObjectInputStream(new FileInputStream(skName));//uploading data from file to array obj skNames
        skNames=(Details[]) ois.readObject();//typecasting obj to class type and again storing in skNames obj array
        ois=new ObjectInputStream(new FileInputStream(skPassword));
        skPasswords=(Details[]) ois.readObject();
        ois=new ObjectInputStream(new FileInputStream(skIDs));
        IDs=(Details[]) ois.readObject();
        ...
        for(i=0;i<sk;i++)
            if(skNames[i].getShopkeeperNames().equals(tname) && skPasswords[i].getShopkeepersPasswords().equals(tpassword) && IDs[i].getIds()==tId){
                ...
                oos=new ObjectOutputStream(new FileOutputStream(productName));  //copying products name to a file obj named productName
                ...
                oos=new ObjectOutputStream(new FileOutputStream(productRates));
                ...
                oos=new ObjectOutputStream(new FileOutputStream(productCounts));
...
public static void customerLogIn(String tname,String tpassword)throws Exception{
        int i;
        ois=new ObjectInputStream(new FileInputStream(customerCount));//copying val back from file to c
        ...
        if(c==0)//if there are no customers registered or no of customers are 0
            ...
            ois=new ObjectInputStream(new FileInputStream(cName));
            cNames=(Details[]) ois.readObject();
            ois.close();
            ois=new ObjectInputStream(new FileInputStream(cPassword));
